...
/*
1-10 Posts라는 클래스 생성(일관되게 Post에 s를 붙인다, 그냥 약속)
    게시판 : 제목, 내용, 작성자 (날짜 등은 나중에 추가할 예정)
    id : 자동으로 결정되기 때문에 id항목은 별도로 필드로 주지 않는다.
    @Entity : JPA에서 필요한 Annotation
        테이블과 클래스 매핑
        Posts.java > posts DB table 을 내부작으로 자동으로 만들어준다.
        MyPost.java > my_post DB table 을 내부적으로 자동으로 만들어준다.
